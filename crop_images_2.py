...
def crop_tiff_images_series(folder=None, series_prefix="", show_message=True):
    ...
    if len(filepaths) == 1:
        # Single image: automatic series filter
        ...
        # Multiple images: crop only the selected images
    ...
    # --- Helper: which side is under cursor ---
    ...
    def on_move(event):
        nonlocal rect
        if not side_being_dragged: return
        x = (event.x - offset_x) / scale
        y = (event.y - offset_y) / scale
        x1, y1, x2, y2 = rect
        if side_being_dragged == "left":
            center_y = (y1 + y2) / 2
            new_side = x2 - x
            rect = [x2 - new_side, center_y - new_side / 2, x2, center_y + new_side / 2]
        elif side_being_dragged == "right":
            center_y = (y1 + y2) / 2
            new_side = x - x1
            rect = [x1, center_y - new_side / 2, x1 + new_side, center_y + new_side / 2]
        elif side_being_dragged == "top":
            center_x = (x1 + x2) / 2
            new_side = y2 - y
            rect = [center_x - new_side / 2, y2 - new_side, center_x + new_side / 2, y2]
        elif side_being_dragged == "bottom":
            center_x = (x1 + x2) / 2
            new_side = y - y1
            rect = [center_x - new_side / 2, y1, center_x + new_side / 2, y1 + new_side]
        draw_canvas()
...
if __name__ == "__main__":
    crop_box = crop_tiff_images_series()
